...
class NFSDataset(BaseDataset):
    ...
    def _construct_sequence(self, sequence_info):
        ...
        # data1 = np.genfromtxt(str(anno_path), delimiter='\t',dtype=str)
        # ground_truth_rect = np.array([[float(value.strip())
        #                                if value.strip() else np.nan for value in row[:8]]
        #                               for row in data1], dtype=np.float64)
        ...
        for row in data1:
            elements = row.split()
            # extracted_numbers = [float(value.strip()) for value in row[1:5]]
            extracted_numbers = elements[1:5]
            ground_truth_rect.append(extracted_numbers)

        ground_truth_rect = np.array(ground_truth_rect, dtype=float64)
